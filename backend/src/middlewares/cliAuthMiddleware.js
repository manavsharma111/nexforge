...
const requireCliAuth = async (req, res, next) => {
  try {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return res
        .status(401)
        .json({ error: "Unauthorized: Missing or invalid CLI token" })
    }
    ...
    if (projectId) {
      ...
      const isCollaborator =
        ...
        project.collaborator.some((c) => c.toString() === user._id.toString())
    ...
    res
      .status(500)
      .json({ error: "Internal Server Error during CLI authentication" })
  }
}
